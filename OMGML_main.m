...
% Colon cancer patients treated with only flurouracil were sourced from The 
% Cancer Genome Atalas. This resulted in n=9 responders and n=2 partial responders. 
% Their tumor transcriptomics were combat normalized and differntially expressed 
% genes relative to HT29 cells were calculated. 
% 
% 
% 
% Load in patient data and merge with existing data 

patient_tbl_partial = readtable('tcga_combat_relative_to_HT29_Fluorouracil_only_PARTIAL_RESPONDERS.tsv','FileType','text','Delimiter','\t');
patient_tbl_resp = readtable('tcga_combat_relative_to_HT29_Fluorouracil_only_RESPONDERS.tsv','FileType','text','Delimiter','\t');

%%%%%%%%%%%%%%% PARTIAL %%%%%%%%%%%%%%%%%%%%%%%%%%%
patient_tbl_partial.Properties.VariableNames{1} = 'gene';
head(patient_tbl_partial)

oldNames = patient_tbl_partial.Properties.VariableNames;

% Prepend 'resp_' to all except the first one
newNames = oldNames;
newNames(2:end) = cellfun(@(x) ...
    ['partial_' regexprep(x, '(Fluorouracil).*', '$1')], ...
    oldNames(2:end), 'UniformOutput', false);

patient_tbl_partial.Properties.VariableNames = newNames;

% Check
head(patient_tbl_partial)


patient_tbl = standardizeMissing(patient_tbl_partial, {'NA','NaN',''});
stripQuotes = @(s) erase(erase(strtrim(string(s)),'"'),'''');
for k = 2:width(patient_tbl)
    if ~isnumeric(patient_tbl.(k))
        patient_tbl.(k) = str2double(stripQuotes(patient_tbl.(k)));
    end
end
% now fill NaNs with 0 in numeric vars
patient_tbl = fillmissing(patient_tbl, 'constant', 0, 'DataVariables', @isnumeric);

% pick only numeric columns (skip the 'gene' key)
numMask   = varfun(@isnumeric, patient_tbl, 'OutputFormat','uniform');
numMask(1) = false;  % assuming col 1 is 'gene'
M = patient_tbl{:, numMask};



merged_tbl = outerjoin(ht29_hct116.added_hct_fuso, patient_tbl, ...
    'Keys', 'gene', 'MergeKeys', true, 'Type', 'left');


% Fill NaNs with 0 in numeric variables only
merged_tbl = fillmissing(merged_tbl, 'constant', 0, 'DataVariables', @isnumeric);

head(merged_tbl)



ht29_hct116.added_hct_fuso = merged_tbl;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RESP %%%%%%%%%%%%%%%%%%
patient_tbl_resp.Properties.VariableNames{1} = 'gene';
head(patient_tbl_resp)

oldNames = patient_tbl_resp.Properties.VariableNames;

% Prepend 'resp_' to all except the first one
newNames = oldNames;
newNames(2:end) = cellfun(@(x) ...
    ['resp_' regexprep(x, '(Fluorouracil).*', '$1')], ...
    oldNames(2:end), 'UniformOutput', false);

patient_tbl_resp.Properties.VariableNames = newNames;

% Check
head(patient_tbl_resp)


patient_tbl = standardizeMissing(patient_tbl_resp, {'NA','NaN',''});
stripQuotes = @(s) erase(erase(strtrim(string(s)),'"'),'''');
for k = 2:width(patient_tbl)
    if ~isnumeric(patient_tbl.(k))
        patient_tbl.(k) = str2double(stripQuotes(patient_tbl.(k)));
...
fold_patient = fold_data(:, patientCols);


% =========================
% Robust-normalize PATIENTS
% =========================
med_p  = median(fold_patient, 2, 'omitnan');
mad_p  = mad(fold_patient, 1, 2);
scale_p = 1.4826 * mad_p;
scale_p(scale_p == 0 | isnan(scale_p)) = NaN;

fold_patient_norm = (fold_patient - med_p) ./ scale_p;

% =========================
% Optional: round for display only (do NOT round if you plan downstream stats)
% =========================
fold_drugs_norm   = round(fold_drugs_norm, 2);
fold_bugs_norm    = round(fold_bugs_norm, 2);
fold_patient_norm = round(fold_patient_norm, 2);

% =========================
% Diagnostics
% =========================
fprintf("Drug range (raw):   [%g, %g]\n", min(fold_drugs(:)),   max(fold_drugs(:)));
fprintf("Patient range(raw): [%g, %g]\n", min(fold_patient(:)), max(fold_patient(:)));
fprintf("Bug range (raw):    [%g, %g]\n", min(fold_bugs(:)),    max(fold_bugs(:)));

mad_all = mad(fold_data, 1, 2);
fprintf("MAD(all) quantiles: %g %g %g\n", quantile(mad_all, [0.1 0.5 0.9]));

fprintf("%% genes with MAD_drugs=0:   %.1f%%\n", 100*mean(mad_d==0));
fprintf("%% genes with MAD_bugs=0:    %.1f%%\n", 100*mean(mad_b==0));
fprintf("%% genes with MAD_patient=0: %.1f%%\n", 100*mean(mad_p==0));

% =========================
% Filter genes to avoid NaNs in any block (strict)
% If you want less strict: only filter per block before struct creation instead.
% =========================
keep = ~isnan(scale_d) & ~isnan(scale_b) & ~isnan(scale_p);

genes_keep = genes(keep);

fold_patient_norm_keep = fold_patient_norm(keep, :);

% =========================
% Create structs (UPDATED)
% =========================

gene_struct_patient = struct( ...
    'genes',      {genes_keep}, ...
    'conditions', {conditions(patientCols)}, ...
    'fold_data',  {fold_patient_norm_keep} );
%% 
% Derive Flux

specs = [1e-2 1e-2 1e-2]; 
thresh = [-0.5, 0.5];
flux_patient = derive_flux(ht29_GEMmodel, gene_struct_patient, 'ModelSpecs', specs, 'Threshold', thresh);
%% 
% Combine with Drug fluxes 

flux_bugs_data  = flux_bugs.flux(:, 4:end);
flux_drugs_data = flux_drugs.flux(:, 4:end);
flux_patient_data = flux_patient.flux(:, 4:end);
metadata  = flux_bugs.flux(:, 1:4);
% Add prefixes to distinguish blocks
flux_patient_data.Properties.VariableNames = matlab.lang.makeUniqueStrings("patient_" + flux_patient_data.Properties.VariableNames);

% Combine into one table
flux_combined = [metadata, flux_drugs_data, flux_patient_data];

% Ensure final uniqueness just in case
flux_combined.Properties.VariableNames = matlab.lang.makeUniqueStrings(flux_combined.Properties.VariableNames);
...
valid_drugs = combined_ps.conditions;
drug1 = train_all.names(:,1);
drug2 = train_all.names(:,2);
is_valid = ismember(drug1, valid_drugs) & ismember(drug2, valid_drugs);
valid_drugs = combined_ps.conditions;
drug1 = train_all.names(:,1);
drug2 = train_all.names(:,2);
is_valid = ismember(drug1, valid_drugs) & ismember(drug2, valid_drugs);
% Filter each field in train_all
train_all_filtered.names = train_all.names(is_valid, :);
train_all_filtered.scores = train_all.scores(is_valid);
key = horzcat(combined_ps.conditions(:), combined_ps.conditions(:));
%% 
% Train with Patient Data 

[training_data, all_model_patient] = caramel(combined_ps, train_all_filtered, ...
    'train', 'MLtype', 'fitrensemble', 'Method', 'Bag', 'Key', key);
%% 
% Predict Patient combinations with flurouracil 

pred_patients = caramel(combined_ps, testStruct, 'predict', ...
            'MLModel', all_model_patient, ...
            'MLtype', 'fitrensemble', ...
            'Method', 'Bag', ...
            'Key', key ...
        );
...
names  = pred_patients.interactionNames(:);
scores = pred_patients.predScores(:);

% Masks
resp_mask    = startsWith(names, 'resp', 'IgnoreCase', true);
partial_mask = startsWith(names, 'partial', 'IgnoreCase', true);

% Keep only resp + partial
keep = resp_mask | partial_mask;
scores_k = scores(keep);

group_k = repmat({''}, sum(keep), 1);
group_k(resp_mask(keep))    = {'resp'};
group_k(partial_mask(keep)) = {'partial'};

% =========================
% Boxplot
% =========================
fig = figure;
boxplot(scores_k, group_k, ...
    'Notch','off', ...
    'Symbol','');   % hide outlier symbols (we'll plot points ourselves)

ylabel('Predicted Scores');
title('Predicted Scores by Patient Type');

% =========================
% Overlay jittered points
% =========================
hold on;
x = strcmp(group_k,'partial')*1 + strcmp(group_k,'resp')*2;
xj = x + 0.06*randn(size(x));   % jitter
plot(xj, scores_k, 'k.', 'MarkerSize',12);
hold off;

% =========================
% Stats
% =========================
[~,p] = ttest2(scores(resp_mask), scores(partial_mask), 'Vartype','unequal');

yl = ylim;
text(1.5, yl(2) - 0.05*(yl(2)-yl(1)), ...
     sprintf('p = %.3g', p), ...
     'HorizontalAlignment','center', ...
     'FontSize',12, 'FontWeight','bold');
saveas(fig, 'predicted_scores_boxplot_patients.pdf');    % PNG image